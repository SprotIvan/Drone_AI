...
DEFAULTS: dict[str, Any] = {
    ...
    "doa_offset_deg": 0.0,
    # ⚠️ `doa_invert` ВИДАЛЕНО ЗВІДСИ (аудит, знахідка H3).
    #
    # Ключ описував напрямок обертання і виглядав як налаштування, але у
    # runtime його не читав НІХТО: конвенцію будує
    # bearing_frame.source_convention(), і вона дивиться лише на
    # `doa_handedness`. Записував його calibrate.py, читав — тільки CLI-
    # діагностика. Оператор міг поставити doa_invert: true, не побачити
    # жодної зміни пеленга і жодного попередження.
    # Гірше: файл міг одночасно містити doa_invert: true і
    # doa_handedness: "CW" — пряме самозаперечення, яке мовчки
    # розв'язувалось на користь CW.
    # Єдине джерело істини про обертання — `doa_handedness` (нижче).
    # save() прибирає застарілий ключ із файлу, щоб він не лишався
    # лежати як чинне налаштування.
    ...
    # ── Атестації, які дозволяють SRP-PHAT (знахідка H5, дефект D5) ──
    #
    # ⚠️ САМ ПЕРЕЛІК mic_channels НЕ Є ДОКАЗОМ. Кортеж (0, 1) у JSON — це
    # твердження оператора, а не вимір: він не доводить, що канали 0 і 1 —
    # сирі мікрофони, а не два оброблені промені XVF3800. Так само кількість
    # каналів нічого не доводить: пристрій може віддавати чотири ОБРОБЛЕНІ
    # канали (напр. 2 промені + 2 опорні).
    #
    # Тому SRP-PHAT вмикається лише коли ОБИДВА прапорці нижче виставлені
    # ЯВНО, і виставляти їх можна ТІЛЬКИ після реального апаратного тесту.
    # Поки вони False, SRP-PHAT вимкнено — це безпечна поведінка за
    # замовчуванням, а не помилка конфігурації.
    #
    # True ставиться тільки після HW-5: постукати по кожному мікрофону і
    # переконатися, що кожен канал реагує окремо (`calibrate.py check`).
    "mic_channels_verified": False,
    # True ставиться тільки після HW-6: виміряти геометрію масиву
    # (штангенциркуль або офіційне креслення саме цієї плати).
    "mic_geometry_verified": False,
...
#: Ключі, які колись писались у файл, але більше нічого не означають.
#: save() прибирає їх при кожному записі — інакше застарілий ключ
#: лишається у файлі й читається людиною як чинне налаштування.
#: Див. знахідку H3 щодо `doa_invert`.
RETIRED_KEYS = ("doa_invert",)
...
def save(cfg: dict[str, Any], path: Path | str = CALIBRATION_PATH) -> None:
    ...
    for key in RETIRED_KEYS:
        if merged.pop(key, None) is not None:
            print(f"   Прибрано застарілий ключ '{key}' — у runtime він не "
                  f"читався; напрямок обертання задає лише doa_handedness.")
