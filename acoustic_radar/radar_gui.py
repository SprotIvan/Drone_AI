...
def draw_target(screen, font, cx, cy, max_radius, px_per_m,
    ...
    if st.angle_ambiguous:
        # ⚠️ ДРУГА ПОЗНАЧКА ПРИБРАНА. Тут малювався маркер на (180 - angle)
        # як «дзеркальний напрямок за двома мікрофонами». Цей кут НІХТО НЕ
        # ВИМІРЮВАВ: для USB-гілки `ambiguous` означає нічию між двома
        # кластерами променів DSP, і справжній другий кластер — інше число
        # (перевірено: промені [300,302,60,62] → пеленг 61°, привид 119°,
        # а виміряний другий кластер 301°). Той самий дефект був у
        # radar_overlay.py. Один вимір = одна позначка; невизначеність
        # чесно повідомляється текстом.
        caption += " (±дзеркально)"
